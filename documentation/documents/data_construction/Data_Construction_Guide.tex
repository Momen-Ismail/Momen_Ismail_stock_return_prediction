...
Development
&
February 1990--December 2019. Used for expanding-window tuning and to refit the
selected final model specifications.
\\

Test
&
January 2020--December 2025. Used for official evaluation after selection.
\\
...
The first tuning fold uses observations through December 2004 and validates on
2005. The window expands annually through the 2019 validation fold. The older
training and validation labels remain only in descriptive fixed-baseline
outputs; they are not a second model-selection split.

\subsection{Expanding-window tuning}

Hyperparameter tuning uses 15 annual expanding-window folds inside the
development period:
...
Through December 2014 & 2015 \\
Through December 2015 & 2016 \\
Through December 2016 & 2017 \\
Through December 2017 & 2018 \\
...
The final specifications are selected using the following procedure:

\begin{enumerate}
  \item retain the original fixed specifications as descriptive baselines;
  \item tune every candidate using the 2005--2019 expanding folds;
  \item select the exact minimum unrounded average monthly MSE in each family;
  \item refit the selected models on the 1990--2019 development sample;
  \item evaluate on the 2020--2025 test period.
\end{enumerate}
...
&
Specification with 2 components.
\\
...
Random Forest
&
Specification with 300 trees.
\\

Gradient Boosting
&
Specification with 100 trees, learning rate 0.01, and maximum depth 1.
\\
...
The fixed-versus-selected table is descriptive only and is not a second
selection stage.
...
\begin{itemize}
  ...
  \item choosing among candidate configurations;
  \item replacing one model with another;
  \item changing a specification after observing test performance.
\end{itemize}

The active Step 02 and Step 03 code does not load the 2020--2025 test sample.
The present report is a revised analysis produced after the earlier test
outcomes had been reviewed, so those observations are not described as a newly
opened holdout for the revision.
...
\begin{center}
\path{documentation/documents/thesis/Momen_Ismail.tex}
\end{center}
