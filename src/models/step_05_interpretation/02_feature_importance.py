...
def tree_group_summary(tree_importance):
    """Summarize tree importance by predictor family."""
    data = tree_importance.copy()

    data["predictor_group"] = (
        data["predictor"]
        .map(utils.predictor_group)
    )
    ...
    summary["nonzero_predictor_share"] = (
        summary["nonzero_predictor_count"]
...
def coefficient_group_summary(coefficients):
    """Summarize linear coefficients by predictor family."""
...
def main():
    ...
    tree_groups = tree_group_summary(
        tree_importance
    )
    ...
    print(
        "Gradient Boosting: "
        f"{len(gradient_boosting_importance)} predictors"
    )
