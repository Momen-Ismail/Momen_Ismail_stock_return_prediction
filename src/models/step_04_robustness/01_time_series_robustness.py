...
def model_factories():
    """Return a fresh optimized Random Forest for each annual refit."""
...
def main():
    ...
    yearly_metrics = []
    for (model, window, year), group in predictions.groupby(
        ["model", "window", "forecast_year"]
    ):
        yearly_metrics.append({
            "model": model,
            "window": window,
            "forecast_year": year,
            "sample": f"validation_{year}",
            "observations": len(group),
            **evaluate_predictions(
                group[TARGET],
                group["prediction"],
    ...
    pd.DataFrame(overall_metrics).to_csv(
        OUTPUT_DIR / "time_series_robustness_metrics.csv",
        index=False,
    )

    pd.DataFrame(yearly_metrics).to_csv(
        OUTPUT_DIR / "time_series_robustness_yearly_metrics.csv",
        index=False,
    )

    predictions.to_parquet(
        OUTPUT_DIR / "time_series_robustness_predictions.parquet",
        index=False,
    )
